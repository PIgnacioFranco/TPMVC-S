...
module.expport = {
	guardarUnaPersona: async persona => {
		let resultado = await conexion.query(
			INSER INT persona (nombre, apellido. nickname, edad, email) VALUES (?,?,?,?,?)',
	...
	borrarPersona: async id => {
		let fecha = new Date();
		let resultado = await conexion.query('UPDATE persona SET deleted = ?, date_teleteç
		let resultado.affectedRows;
	}
}
